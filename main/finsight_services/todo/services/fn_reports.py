...
reports_id_class = ['01','02','10','11','20','21','22',
...
def get_ecn_reports(driver: webdriver.Chrome,result,year,quarter) -> str:
    '''
    ...
    - year: năm báo cáo
    - quarter: giới hạn quý hiện tại
    '''
    try:
        link = f"https://cafef.vn/du-lieu/bao-cao-tai-chinh/{result}/incsta/{year}/{quarter}/0/0/1/ket-qua-hoat-dong-kinh-doanh.chn"
        driver.get(link)
        return 'Access Finance reports'
    except Exception as error:
        print("ECN report Exception : ",error)
        return
    


#lấy theo danh mục
def convert_sales_service_data_reports(driver:webdriver.Chrome):
    try:
        '''
...
def current_timeline_reports(driver:webdriver.Chrome):
...
def convert_data_reports(id:str,driver:webdriver.Chrome):
    '''
...
def data_loops_reports(tube:list,driver:webdriver.Chrome):
    try:
        data = []

        for obj in range(len(tube)):  
            tt,q1,q2,q3,q4 = convert_data_reports(tube[obj],driver=driver)
            data.append(
                {
                    "title":tt,
                    "First":q1,
        ...
        print(f"Reports loop errors : ",error)
        return []
...
#         # btn.click()
#         time.sleep(3)
#         print(get_ecn_reports(driver,result))
#         print("Check Result .... ")
