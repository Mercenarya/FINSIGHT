...
async def get_finance_reports(driver,result,year,quarter):
    try:
        ...
        time.sleep(3)
        get_ecn_reports(driver,result=result,year=year,quarter=quarter)
        print("Check Result .... ")
      
        time.sleep(5)
        convert_sales_service_data_reports(driver)
        time.sleep(3)
        current_timeline_reports(driver)
        time.sleep(3)
        ...
        time.sleep(3)
        convert_data_frame(tt,qq1,qq2,qq3,qq4,RAW)
...
async def get_finance_assets(driver,result,year,quarter):
    try:
        wait = WebDriverWait(driver,10)
        ...
        # btn.click()
        time.sleep(3)
        get_ecn_assets(driver,result=result,year=year,quarter=quarter)
...
async def run_procedure_collect(driver,result,year,quarter):
    # reports
    url = 'https://cafef.vn/du-lieu.chn'
    driver.execute_script("window.open('about:blank','_blank');")
    driver.switch_to.window(driver.window_handles[-1])
    driver.get(url)
    await get_finance_reports(driver=driver,result=result,year=year,quarter=quarter)
    ...
    driver.switch_to.window(driver.window_handles[-1])
    driver.get(url)
    await get_finance_assets(driver=driver,result=result,year=year,quarter=quarter)
