...
def get_ecn_assets(driver: webdriver.Chrome, result:str, year:int, quarter:int) -> str:
    '''
    ...
    '''
    try:
        url = f"https://cafef.vn/du-lieu/bao-cao-tai-chinh/{result}/bsheet/{year}/{quarter}/0/0/1/ket-qua-hoat-dong-kinh-doanh.chn"
        driver.get(url=url)
...
def data_loops_assets(tube:list,driver:webdriver.Chrome):
    try:
        ...
        print(f"Data loops error: {error}")
        return []
